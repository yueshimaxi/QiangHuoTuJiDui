...
void ANA_GraphPanel::ComputeArrangeGraph(bool _bCompact, bool _bCenterize, const FGraphConfig& _graphConfig)
{
	...
	case OnlySelected:
	case OnlyComments:
		ANA_ReleaseNotifier::CompleteNotification(CompleteActionType::ARRANGE_SELECTED_NODES);
		bSelectedNodeOrComments = true;
		break;
	case All:
		ANA_ReleaseNotifier::CompleteNotification(CompleteActionType::ARRANGE_ALL_NODES);
		bSelectedNodeOrComments = false;
		break;
	case Connected:
		ANA_ReleaseNotifier::CompleteNotification(CompleteActionType::ARRANGE_CONNECTED_NODES);
		bSelectedNodeOrComments = false;
		break;
	default:
		bSelectedNodeOrComments = false;
