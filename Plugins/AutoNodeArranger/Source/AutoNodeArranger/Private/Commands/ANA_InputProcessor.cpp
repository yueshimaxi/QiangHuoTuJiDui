...
void ANA_InputProcessor::UpdatePinLinkedTo()
{
	...
	if (!currentGraphPin)
		return;
	if (currentGraphPin->bWasTrashed)
	{
		currentGraphPin = nullptr;
		return;
	}
	if (currentGraphPin->LinkedTo != oldLinkedTo)
	{
		if (bControlDown)
		{
			oldLinkedTo = currentGraphPin->LinkedTo;
			bControlDown = false;
			return;
		}
			countdownTickCount--;
		if (countdownTickCount == 0)
		{
			...
			for (auto connectedToPin : currentGraphPin->LinkedTo)
			...
			nodeToSelectSet.Add(currentGraphPin->GetOwningNodeUnchecked());
			...
			graphPanel->SelectionManager.SetNodeSelection(currentGraphPin->GetOwningNodeUnchecked(), true);
			oldLinkedTo = currentGraphPin->LinkedTo;
		}
...
void ANA_InputProcessor::UpdateCurrentGraphAndPin(const FWidgetPath& widgetsUnderCursor)
{
	...
	if (weakSGraphPanel.IsValid())
	{
		...
		{
			UEdGraphPin* graphPin = weakGraphPin.Pin()->GetPinObj();
			...
			{
				currentGraphPin = graphPin;
				oldLinkedTo.Append(currentGraphPin->LinkedTo);
			}
		}
	}
}
